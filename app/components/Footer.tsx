...
export default function Footer() {
  return (
    <footer className="mt-auto border-t border-emerald-900 bg-green-950 px-6 py-8 text-emerald-50">
      <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-[1.2fr_0.8fr]">
        ...
        <nav className="flex flex-wrap content-start gap-2 md:justify-end" aria-label="Footer navigation">
          {footerLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full border border-emerald-800 bg-emerald-950/70 px-3 py-2 text-sm font-semibold text-emerald-50 transition hover:border-yellow-300 hover:text-yellow-100 focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:ring-offset-2 focus:ring-offset-green-950"
            >
              {link.label}
            </Link>
          ))}
